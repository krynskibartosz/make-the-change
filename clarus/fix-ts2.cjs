...
rep(
  ...
  /billingStatus:\s*'[^']*',\s*paymentStatus:\s*'[^']*',/g,
  '',
)
rep(
...
rep(
  `${base}\\src\\features\\interventions\\add-flow\\step-status.tsx`,
  /import \{ ChoiceCard \} from '@\/components\/ui'/,
  'import { Card as ChoiceCard } from "@/components/ui"',
)
rep(
  `${base}\\src\\features\\interventions\\add-flow\\step-what.tsx`,
  /import \{ ChoiceCard \} from '@\/components\/ui'/,
  'import { Card as ChoiceCard } from "@/components/ui"',
)
rep(
  `${base}\\src\\features\\interventions\\add-flow\\step-where.tsx`,
  /import \{ ChoiceCard \} from '@\/components\/ui'/,
  'import { Card as ChoiceCard } from "@/components/ui"',
)
rep(
...
rep(
  ...
  /updateInterventionDraft/g,
  'createInterventionDraft',
)

rep(
  `${base}\\src\\lib\\repositories\\mock-clarus-repository.ts`,
  /const updated: Person = \{ \.\.\.person, \.\.\.input, id: person\.id, projectId: person\.projectId, name: input\.name \|\| person\.name, defaultHourlyRate: input\.defaultHourlyRate \?\? person\.defaultHourlyRate \}/g,
