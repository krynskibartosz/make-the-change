...
export default function CostsPage() {
  ...
  const laborPercentage = dashboard.totalCost > 0 
    ? Math.round((dashboard.laborAmount / dashboard.totalCost) * 100) 
    : 0
          ...
          <div className="bg-surface rounded-2xl border border-border p-5 shadow-sm mb-4">
            ...
            <div className="mt-2 flex items-center justify-between text-xs">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-muted-foreground">Achats & Matériaux ({100 - laborPercentage}%)</span>
              </div>
            </div>
          </div>
                ...
                <div key={expense.id} className="flex items-center justify-between p-4 border-b border-border last:border-0">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      {expense.supplier && (
                        <>
                          ...
                          <span className="text-xs text-muted-foreground truncate">{expense.supplier}</span>
                        </>
                      )}
                    </div>
                  </div>
                  <div className="flex flex-col items-end shrink-0">
                    ...
                    {expense.isRebillable && expense.status !== 'invoiced' && (
                      <span className="text-[10px] font-bold uppercase text-amber-500 mt-1">À facturer</span>
                    )}
                  </div>
                </div>
              ))}
