...
import type { Project, Client, DashboardKPIs } from '@/lib/domain'
import { ChevronLeft, Building2, MapPin, User, FileText, Calendar, Phone, Mail } from 'lucide-react'
    ...
    <div className="flex flex-col min-h-dvh bg-background text-foreground pb-20">
      ...
      <header className="sticky top-0 z-30 flex flex-col gap-4 p-5 pb-4 pt-[max(env(safe-area-inset-top),1.25rem)] bg-background/80 backdrop-blur-md border-b border-border/30">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            ...
            <p className="text-xs text-muted-foreground uppercase tracking-wider">{project.status === 'active' ? 'En cours' : project.status}</p>
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 p-5 flex flex-col gap-6">
        ...
        <div className="bg-surface border border-border rounded-2xl p-5 flex flex-col gap-4">
          <h3 className="font-bold text-sm uppercase tracking-wider text-muted-foreground">Détails Techniques</h3>
          ...
          <div className="flex gap-3">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-muted-foreground text-sm leading-relaxed">{project.description}</span>
            </div>
          </div>
        ...
        {client && (
          <div className="bg-surface border border-border rounded-2xl p-5 flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-sm uppercase tracking-wider text-muted-foreground">Client Associé</h3>
              <Link href={`/client/${client.id}`} className="text-xs text-primary font-semibold bg-primary/10 px-2 py-1 rounded-md">
                Voir profil complet
              </Link>
            </div>
            ...
            <div className="flex gap-3 items-center">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-muted-foreground text-sm">{client.company || 'Particulier'}</span>
              </div>
            </div>

            <div className="flex gap-2 mt-2">
              <a href={`tel:${client.phone}`} className="flex-1 flex items-center justify-center gap-2 bg-surface-elevated hover:bg-border/50 py-2 rounded-xl text-sm font-medium transition-colors">
                <Phone className="size-4" /> Appeler
              </a>
              <a href={`mailto:${client.email}`} className="flex-1 flex items-center justify-center gap-2 bg-surface-elevated hover:bg-border/50 py-2 rounded-xl text-sm font-medium transition-colors">
        ...
        {kpis && (
          <div className="bg-surface border border-border rounded-2xl p-5 flex flex-col gap-4">
             <h3 className="font-bold text-sm uppercase tracking-wider text-muted-foreground">Budget & Temps</h3>
             <div className="grid grid-cols-2 gap-4">
               <div className="flex flex-col gap-1">
                 <span className="text-2xl font-bold">{Math.round((kpis.totalHours / kpis.budgetHours) * 100)}%</span>
                 <span className="text-xs text-muted-foreground">Budget heures consommé</span>
               </div>
               <div className="flex flex-col gap-1">
                 <span className="text-2xl font-bold">{Math.round((kpis.totalCost / kpis.budgetCost) * 100)}%</span>
