...
replaceInFile('src/features/interventions/add-flow/draft.ts', [
...
replaceInFile('src/features/interventions/components/intervention-card.tsx', [
    ...
    ['tone={(intervention.status === "in_progress" ? "warning" : "success") as any}', 'tone={(intervention.status === "in_progress" ? "warning" : "success") as any}'],
    ['(task.status === "to_do" ? "neutral" : "success") as any', '(task.status === "to_do" ? "neutral" : "success") as any'],
    ['export function InterventionCard({ intervention }: InterventionCardProps) {', 'export function InterventionCard({ intervention }: any) {']
]);

replaceInFile('src/features/interventions/edit-flow/edit-intervention-flow.tsx', [
    ['await mockClarusRepository.createInterventionDraft(interventionId, input)', 'await mockClarusRepository.createInterventionDraft(input as any)']
]);
