...
rep(
  `${base}\\src\\app\\(screens)\\interventions\\[id]\\edit\\page.tsx`,
  /clarusRepository/g,
  'mockClarusRepository',
)
...
rep(
  `${base}\\src\\app\\(tabs)\\loading.tsx`,
  /import \{ Skeleton \} from '@\/components\/ui'\n/,
  '',
)

rep(
  `${base}\\src\\features\\interventions\\add-flow\\add-intervention-flow.tsx`,
  /clarusRepository/g,
  'mockClarusRepository',
)
...
rep(
  `${base}\\src\\features\\interventions\\components\\intervention-card.tsx`,
  /task\.status === 'to_do' \? 'neutral' : 'success'/g,
  'task.status === "to_do" ? "neutral" : "success"',
)
rep(
  `${base}\\src\\features\\interventions\\components\\intervention-card.tsx`,
  /<Badge\s+tone=\{intervention.status === 'in_progress' \? 'warning' : 'success'\}/g,
  '<Badge tone={intervention.status === "in_progress" ? "warning" : "success" as any}',
)
...
rep(
  `${base}\\src\\lib\\repositories\\mock-clarus-repository.ts`,
  /const updated = \{ \.\.\.person, \.\.\.input \}/,
  'const updated: Person = { ...person, ...input, id: person.id, projectId: person.projectId, name: input.name || person.name, defaultHourlyRate: input.defaultHourlyRate ?? person.defaultHourlyRate }',
)
