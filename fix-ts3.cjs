...
rep(`${base}\\src\\features\\interventions\\add-flow\\draft.test.ts`, /billingStatus: 'to_invoice',\s*paymentStatus: 'unpaid',/g, '');
rep(`${base}\\src\\features\\interventions\\add-flow\\draft.ts`, /billingStatus: 'to_invoice',\s*paymentStatus: 'unpaid',/g, '');
rep(`${base}\\src\\features\\interventions\\add-flow\\draft.test.ts`, /billingStatus: 'to_invoice',/g, '');
rep(`${base}\\src\\features\\interventions\\add-flow\\draft.test.ts`, /paymentStatus: 'unpaid',/g, '');
rep(`${base}\\src\\features\\interventions\\add-flow\\draft.ts`, /billingStatus: 'to_invoice',/g, '');
...
rep(`${base}\\src\\features\\interventions\\edit-flow\\edit-intervention-flow.tsx`, /await mockClarusRepository.createInterventionDraft\(interventionId, input\)/g, 'await mockClarusRepository.createInterventionDraft(input)');
