...
import { type SupportedLanguage, SUPPORTED_LANGUAGES, getTranslationCompleteness } from '@make-the-change/shared';
...
export const TranslationProvider: FC<TranslationProviderProps> = ({
  ...
  const setFieldTranslation = useCallback((fieldName: string, language: SupportedLanguage, value: string) => {
    console.log(`[TranslationContext] setFieldTranslation: ${fieldName} [${language}] = "${value}"`);
    ...
    setTranslations((prev) => {
      ...
      console.log(`[TranslationContext] Updated translations for ${fieldName}:`, updated[fieldName]);
      return updated;
    });
    setIsDirty(true);
  }, []);
  ...
  const getFieldTranslation = useCallback(
    (fieldName: string, language: SupportedLanguage): string => {
      ...
      console.log(`[TranslationContext] getFieldTranslation: ${fieldName} [${language}] = "${value}"`);
      return value;
    },
    [translations]
  );
