      ...
      for (const file of files) {
        ...
        try {
          const blurResponse = await fetch(
            `${supabaseUrl}/functions/v1/generate-blur-hash`,
            {
              method: 'POST',
              headers: {
  ...
  try {
    ...
    try {
      ...
      if (blurResponse.ok) {
        ...
        if (blurResult.success) {} else {
          console.warn(
            `⚠️ Blur generation failed pour remplacement: ${blurResult.error}`
          );
        }
      } else {
        console.warn(
      ...
      console.error(
        `❌ Blur generation error pour remplacement ${newImageUrl}:`,
        blurError
      );
