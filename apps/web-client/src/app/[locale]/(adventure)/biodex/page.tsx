...
export default function BiodexPage({ params }: { params: { locale: string } }) {
  ...
  const [species, setSpecies] = useState<SpeciesContext[]>([])

  // Mock data - in production this would be fetched server-side
  const session = getClientMockViewerSession()
  const initialFaction = session?.faction || null
  const hasFaction = !!initialFaction
  ...
  const handleFactionSelect = async (faction: { id: string; value: string; name: string; mascotName: string; description: string; colorTheme: string; accentColor: string; accentText: string; buttonText: string; image: string }) => {
    ...
    formData.append('returnTo', '/adventure/biodex')
        ...
        <div className='fixed inset-0 z-[80]'>
          ...
          <div className='absolute inset-x-0 bottom-0 max-h-[88dvh] overflow-y-auto rounded-t-3xl border border-white/10 bg-[#0B0F15] p-5 pb-[max(1.25rem,env(safe-area-inset-bottom))]'>
            ...
            <Link
              ...
              onClick={() => setSelectedLockedSpecies(null)}
              className={cn(
                'mt-5 flex h-12 w-full items-center justify-center gap-2 rounded-2xl text-base font-black text-black active:scale-[0.98]',
                accentTheme.accentBg,
              )}
            >
              Voir les projets <Sparkles className='h-4 w-4' />
            </Link>
          </div>
        </div>
      ) : null}
