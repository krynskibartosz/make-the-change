import { CheckCircle2, Flame, Sprout, Target, Trophy, Zap } from 'lucide-react'
...
export async function AdventureChallenges() {
	...
	const items = challengeRows.map((c) => {
		...
		const target =
			c.title.toLowerCase().includes('7 jours') ||
			c.title.toLowerCase().includes('série')
				? 7
		...
		const getIcon = () => {
			if (
				c.title.toLowerCase().includes('série') ||
				c.title.toLowerCase().includes('streak')
			) {
				return Flame
			}
	...
	return (
		<div className='gap-8 w-full  animate-in fade-in slide-in-from-bottom-2 duration-500'>
			{items.length === 0 ? (
				<div className='flex w-full min-h-[40vh] items-center justify-center rounded-[3rem] border-2 border-dashed bg-muted/20 p-12 text-center'>
						...
						return (
							...
							>
								...
								<div className='w-12 h-12 rounded-xl flex-shrink-0 flex items-center justify-center bg-white/10 group-hover:bg-lime-500/20 transition-colors'>
									<Icon
										size={24}
										className='text-white group-hover:text-lime-400 transition-colors'
									/>
								</div>
								...
								<div className='flex-1 flex flex-col gap-1.5 min-w-0'>
									...
									<div className='flex items-center gap-2'>
										<h3 className='text-base font-semibold text-foreground tracking-tight truncate'>
											{c.title === 'Streak 7 jours' ||
											c.title === 'Série 7 jours'
												? 'Série de 7 jours'
												: c.title}
										</h3>
								...
								<div className='flex-shrink-0 text-right'>
									{c.userProgress.isClaimed ? (
										...
										<span className='font-bold text-lime-400 text-sm tabular-nums'>
											+{c.reward_graines} 🌱
										</span>
									) : (
										<div className='flex items-center gap-1 text-amber-400 font-bold'>
											<Trophy className='w-4 h-4' />
											<span className='uppercase text-[10px] tracking-wider font-bold text-amber-400'>
												BADGE
											</span>
										</div>
									)}
