  ...
  return (
    <div className="flex items-start gap-3 py-3 border-b border-white/5 last:border-0">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-sm text-foreground/75 leading-relaxed mt-0.5">{event.action}</p>
      </div>
      <button
        onClick={handleBravo}
        className={cn(
...
export function GuildDetailClient({
  ...
  const missionPercent = activeMission
    ? Math.min(100, Math.round((activeMission.goal_current / activeMission.goal_target) * 100))
    : null
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background pb-24">
      ...
      {/* ── A. HERO IMMERSIF ─────────────────────────────────────── */}
      <div
        className="relative mt-3 mx-4 rounded-3xl overflow-hidden min-h-[220px] flex flex-col justify-end"
        style={
          bannerUrl
            ? {
      ...
      >
        ...
        <div className="relative z-10 p-5 pb-6">
          {/* Squircle Avatar */}
          <div className="h-14 w-14 rounded-2xl bg-lime-500/20 border border-lime-500/30 flex items-center justify-center mb-3 overflow-hidden">
            <span className="text-xl font-black text-lime-300">{guildName[0]}</span>
          </div>
      ...
      <div className="mx-4 mt-4 rounded-3xl bg-lime-950/40 border border-lime-500/20 p-5">
        {activeMission ? (
          <>
            ...
            <p className="text-base font-bold text-white mb-1">{activeMission.goal_label}</p>

            {/* Jauge */}
            <div className="h-3 w-full bg-white/10 rounded-full overflow-hidden mb-1">
              <div
                className="h-full bg-gradient-to-r from-lime-500 to-emerald-400 rounded-full transition-all duration-700"
                style={{ width: `${missionPercent}%` }}
              />
            </div>
            <div className="flex justify-between items-center mb-4">
              <span className="text-xs text-muted-foreground tabular-nums">
                {activeMission.goal_current.toLocaleString()} / {activeMission.goal_target.toLocaleString()} {activeMission.goal_unit}
              </span>
              <span className="text-xs font-bold text-lime-400 tabular-nums">{missionPercent}%</span>
            </div>

            {/* Récompense */}
            <div className="flex items-center gap-2 bg-white/5 rounded-xl px-3 py-2 mb-4 border border-white/10">
              <span className="text-xl">{activeMission.reward_species_emoji}</span>
              <div>
          ...
          <div className="text-center py-4">
            ...
            <p className="font-semibold text-foreground">Aucune mission active</p>
            <p className="text-sm text-muted-foreground mt-1">La prochaine mission arrive bientôt.</p>
          </div>
        )}
      </div>
      ...
      <div className="mx-4 mt-4 flex items-center gap-3 px-1">
          ...
          <div className="flex">
            {avatarStack.map((member, i) => (
              <div
                key={member.user_id}
                className={cn(
                  'h-8 w-8 rounded-full border-2 border-background bg-emerald-500/20 flex items-center justify-center text-xs font-bold text-emerald-400',
                  i > 0 && '-ml-2',
                )}
        ...
        <div className="flex items-center gap-1.5 text-sm text-muted-foreground">
          ...
          <span>
            <span className="font-bold text-foreground">{membersCount.toLocaleString()}</span> membres actifs
          </span>
        </div>
      </div>
      ...
      {/* ── CTA flottant Rejoindre (non-membre uniquement) ───────── */}
      {!isMember && (
