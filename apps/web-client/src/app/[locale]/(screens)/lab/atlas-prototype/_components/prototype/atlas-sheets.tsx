...
import { TerritoryIcon } from './atlas-territories'
...
export function TerritorySheet({ territory, onClose }) {
  ...
  React.useEffect(() => {
    if (territory) setLast(territory)
  }, [territory])
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
              {t.stats.map(([k, v]) => (
                ...
                >
                  ...
                  <div style={{ fontSize: 11, color: '#9a937f', marginTop: 2, letterSpacing: 0.3 }}>
                    {k}
                  </div>
                </div>
              ))}
...
export function SearchPanel({ open, onClose, onPickQuery }) {
  ...
  const filtered = q
    ? suggestions.filter((s) => (s.label + ' ' + s.kind).toLowerCase().includes(q.toLowerCase()))
    : suggestions
...
export function GuideOverlay({ open, onClose }) {
