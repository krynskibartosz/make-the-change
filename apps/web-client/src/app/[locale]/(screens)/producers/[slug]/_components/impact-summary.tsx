  ...
  return (
    <section className="mt-10 px-5">
      <div className="rounded-2xl border border-white/5 bg-gradient-to-br from-amber-500/5 to-transparent p-5">
        {/* Label prudent */}
        <p className="text-[12px] font-bold uppercase tracking-[0.09em] text-amber-400/65">
          {summary.wording}
        </p>
        ...
        <div className="mt-2 flex items-baseline gap-1">
          ...
          <span className="text-sm font-bold text-amber-400/60">{summary.unit}</span>
        </div>

        {/* Disclaimer */}
        <div className="mt-4 flex items-start gap-2 rounded-lg bg-white/5 p-3">
          <Info className="h-4 w-4 shrink-0 text-white/40" />
          <p className={typo.modalBodyMuted}>{summary.disclaimer}</p>
        </div>
      </div>
    </section>
  )
