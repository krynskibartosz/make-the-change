...
export async function generateMetadata({ params }: LearningCourseDetailPageProps): Promise<Metadata> {
  const { courseId } = await params
  const course = getLearningCourseById(courseId)

  return {
    title: course ? `${course.title} | Apprendre | Make the Change` : 'Cours non trouvé | Make the Change',
  }
}
            ...
            <div className="mt-3 flex flex-col gap-2">
              {course.relatedProjectSlugs.map((slug) => (
                <Link
                  key={slug}
                  href={`/projects/${slug}`}
                  className="flex items-center justify-between rounded-2xl bg-white/[0.045] px-3 py-2 text-[13px] font-semibold text-white/72 active:bg-white/[0.07]"
                >
              ...
              {course.relatedProjectSlugs.length === 0 && (
                <p className="text-[13px] leading-relaxed text-white/42">
          ...
          <div className="rounded-[1.35rem] border border-white/8 bg-white/[0.035] p-4">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {[course.subject, ...course.tags.slice(0, 8)].map((tag) => (
                <span key={tag} className="rounded-full bg-white/[0.06] px-2.5 py-1 text-[11px] font-semibold text-white/48">
                  {tag}
                </span>
              ))}
            </div>
