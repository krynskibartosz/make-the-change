...
import { getSeasonProgress } from "@/app/[locale]/(tabs)/impact/_lib/mock-seasons";
...
import {
  AdventureTab,
  type AdventureQuestCard,
} from "./_features/adventure-tab";
...
function getSpeciesForProject(
  ...
  return (
    species.find((entry) =>
      entry.associated_projects?.some(
...
export default async function AdventurePage() {
  ...
  const [challengeSurface, projects, speciesList, seeds, impactPoints] =
    await Promise.all([
      isMockDataSource
      ...
      viewerId
        ? getCurrentMockWalletBalance(viewerId, faction)
        : Promise.resolve(0),
      viewerId
        ? getCurrentMockImpactPoints(viewerId, faction)
        : Promise.resolve(0),
    ]);
  ...
  const recommendedProject =
    ...
    projects[0] ??
    null;
  const linkedSpecies = getSpeciesForProject(
    speciesList,
  ...
  const primaryQuest =
    quests.find((quest) => quest.progress < quest.max) ?? quests[0] ?? null;
      ...
      <div className="relative w-full">
        <Suspense fallback={fallbackLoader}>
          <AdventureTab
            ...
            monthlyObjective={
              monthlyQuest?.objective ?? "Explorer le vivant chaque semaine"
            }
            ...
            recommendedProject={
              recommendedProject
                ? {
                    name:
                      recommendedProject.name_default ?? "Projet biodiversite",
                    description:
                    ...
                    typeLabel:
                      recommendedProject.type === "donation"
                        ? "Don pur"
                        : "Soutien producteur",
