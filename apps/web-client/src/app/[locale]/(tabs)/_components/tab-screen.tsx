...
export function TabScreen({ header, children, className, contentClassName }: TabScreenProps) {
  return (
    <div className={cn('fixed inset-0 z-40 bg-[#0B0F15]', className)}>
      {header && (
        <header className="fixed left-0 right-0 top-0 z-50 border-b border-white/5 bg-background/80 px-4 pt-[max(0.75rem,env(safe-area-inset-top))] backdrop-blur-lg">
      ...
      <div
        className={cn(
          ...
          header ? 'pt-[calc(3rem+max(0.75rem,env(safe-area-inset-top)))]' : 'pt-0',
