...
import { Home, Plus, User, Flame, Users } from 'lucide-react'
...
export function CommunityMobileBottomNav({ user }: CommunityMobileBottomNavProps) {
    ...
    const navItems = [
        ...
        },
        {
            ...
            icon: Flame,
            label: tCommunity('sidebar.trending') || 'Tendances',
            isActive: pathname.startsWith('/community/trending'),
        },
        {
            href: '/community/posts/new',
            icon: Plus,
            label: tCommunity('sidebar.update_post') || 'Publier',
        ...
        },
        {
            ...
            icon: Users,
            label: tCommunity('sidebar.guilds') || 'Guildes',
    ...
    return (
        <div className="fixed bottom-0 left-0 right-0 z-50 border-t bg-background/80 backdrop-blur-lg pb-safe md:hidden">
            <nav aria-label="Navigation mobile communauté">
                                ...
                                >
                                    <div
                                        className={cn(
                                            ...
                                            item.isAction ? 'bg-primary text-primary-foreground' : (item.isActive ? 'bg-primary/10' : 'bg-transparent'),
                                        )}
                                    >
                                        <Icon className={cn('h-5 w-5', !item.isAction && item.isActive && 'fill-primary/20')} />
                                    </div>
