    ...
    <div className="flex h-screen bg-background">
      ...
      <div className="relative hidden lg:flex lg:w-[45%] xl:w-[50%] flex-col justify-between overflow-hidden p-16 h-full">
        ...
        <div className="absolute inset-0 -z-10">
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-br from-client-slate-950 via-client-slate-900/90 to-primary/40" />
        </div>

        {/* Animated Accents */}
        <div className="absolute top-[-10%] left-[-10%] w-[60%] h-[60%] rounded-full bg-primary/20 blur-[120px] animate-pulse" />
        <div className="absolute bottom-[-10%] right-[-10%] w-[50%] h-[50%] rounded-full bg-client-emerald-500/10 blur-[100px]" />
        ...
        <Link href="/" className="relative z-10 flex items-center gap-3 group">
          <Logo
            ...
            height={48}
          />
          <span className="text-2xl font-black text-client-white transition-transform group-hover:scale-105 duration-500">
            Make the Change
          </span>
        </Link>
        ...
        <div className="relative z-10 space-y-8 max-w-xl">
          <div className="space-y-6">
            ...
            <h1 className="text-5xl xl:text-7xl font-black leading-[1.05] text-client-white tracking-tighter">
              {t('layout.title_prefix')}{' '}
              <span className="text-primary">{t('layout.title_highlight')}</span>{' '}
              {t('layout.title_suffix')}
            </h1>
            <p className="text-xl text-client-slate-300 font-medium leading-relaxed">
          ...
          <div className="grid gap-6">
            {features.map((feature, i) => (
              <div key={i} className="flex items-center gap-4 group">
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-client-white/5 backdrop-blur-xl border border-client-white/10 transition-colors group-hover:bg-client-white/10">
                  <feature.icon className={cn('h-6 w-6', feature.color)} />
                </div>
                <span className="text-lg font-bold text-client-white/90">{feature.text}</span>
              </div>
            ))}
          </div>
      ...
      <div className="relative flex flex-1 flex-col items-center justify-center px-6 py-8 lg:px-12 lg:py-6 xl:px-24 xl:py-4 bg-gradient-to-br from-background via-muted/20 to-client-emerald-50/20 dark:via-muted/5 dark:to-client-emerald-950/10 h-full overflow-hidden">
        ...
        <div className="mt-12 flex flex-col items-center gap-4 lg:hidden">
          <div className="flex -space-x-3">
            ...
            <div className="flex h-10 w-10 items-center justify-center rounded-full border-2 border-background bg-primary text-client-white text-[10px] font-black shadow-lg">
              +10K
            </div>
          </div>
