...
import { ArrowLeft, Check, CreditCard, Leaf, Lock, ShieldCheck, TrendingUp } from 'lucide-react'
...
export function InvestClient({ project }: InvestClientProps) {
  ...
  return (
    <div className="min-h-[80vh] flex flex-col items-center justify-center py-12 px-4 relative overflow-hidden">
      ...
      <div className="absolute top-1/4 right-[-10%] w-96 h-96 bg-marketing-positive-500/10 rounded-full blur-3xl" />
      <div className="absolute bottom-1/4 left-[-10%] w-96 h-96 bg-marketing-info-500/10 rounded-full blur-3xl" />
      ...
      <div className="w-full max-w-5xl space-y-8 relative z-10">
        ...
        <Card className="border-border/50 shadow-2xl shadow-primary/5 overflow-hidden backdrop-blur-xl bg-background/80">
          <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-primary via-marketing-positive-400 to-primary" />
          ...
          <CardContent className="space-y-8 pt-6">
            ...
            {numericAmount >= 10 && (
              <div className="bg-marketing-positive-500/10 border border-marketing-positive-500/20 rounded-xl p-4 flex items-center gap-4 animate-in fade-in slide-in-from-bottom-2 duration-500">
                <div className="h-10 w-10 rounded-full bg-marketing-positive-500/20 flex items-center justify-center shrink-0">
                  <TrendingUp className="h-5 w-5 text-marketing-positive-600 dark:text-marketing-positive-400" />
                </div>
                <div>
                  <p className="text-sm font-bold text-marketing-positive-700 dark:text-marketing-positive-300">
                    {t('impact_title')}
                  </p>
                  <p className="text-xs text-marketing-positive-600/80 dark:text-marketing-positive-400/80">
            ...
            <div className="space-y-2">
              ...
              <div className="h-2 w-full bg-muted rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${((project.current_funding || 0) / (project.target_budget || 1)) * 100}%`,
                  }}
                />
                <div
              ...
              {numericAmount > 0 && (
                <p className="text-[10px] text-center text-muted-foreground">
                  {t('contribution_add', {
                    percent: ((numericAmount / (project.target_budget || 1)) * 100).toFixed(2),
                  })}
                </p>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="flex items-center gap-2 text-muted-foreground">
                ...
                <span className="text-xs">{t('secure_payment')}</span>
              </div>
              <div className="flex items-center gap-2 text-muted-foreground">
                <ShieldCheck className="h-4 w-4" />
                <span className="text-xs">{t('verified_project')}</span>
              </div>
            </div>
          </CardContent>
