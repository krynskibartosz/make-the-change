...
type HomeSectionVariant = 'default' | 'muted'
...
export const generateMetadata = async ({
 ...
 const HomePage =  async () => {
   ...
   const {
     user, homeContent, activeProjectsState, activeProductsState, membersCountState, pointsGeneratedState, featuredProjectsState, featuredProductsState, activeProducersState, blogPostsState,
   } = await getHomeServerData()

   const partnerAppBaseUrl = process.env.NEXT_PUBLIC_PARTNER_APP_BASE_URL || process.env.NEXT_PUBLIC_ADMIN_URL
   const partnerDashboardUrl = buildPartnerDashboardUrl(partnerAppBaseUrl, locale)

   const heroContextualCta = user
     ? { href: '/projects', label: t('steps.invest.title') }
     : { href: '/register', label: t('cta.button') }

   const heroStats: HeroStat[] = []
   if (activeProjectsState.status === 'ready') heroStats.push({
     key: 'projects',
     href: '/projects',
     icon: Zap,
   ...
   if (membersCountState.status === 'ready') heroStats.push({
     key: 'members',
     href: '/leaderboard',
     icon: Users,
   ...
   const statsItems = []
   if (activeProjectsState.status === 'ready') statsItems.push({
     value: activeProjectsState.value,
   ...
   if (membersCountState.status === 'ready') statsItems.push({
     value: membersCountState.value,
   ...
   if (activeProductsState.status === 'ready') statsItems.push({
   ...
   if (pointsGeneratedState.status === 'ready') statsItems.push({
   ...
   const featuredSectionsVisibleCount = Number(featuredProjectsVisible) + Number(featuredProductsVisible)
   ...
   const primaryCta = user
     ? { href: '/projects', label: 'Investir dans des projets' }
     : { href: '/register', label: 'Nous rejoindre' }
   ...
   return (
     <>
       ...
       <PageHero.Layout
         size="lg"
         variant="gradient"
         className="relative flex min-h-dvh flex-col justify-center pt-24 md:pt-28"
       >
         <PageHero.Badge>
       ...
       <HomeUniverseSection
         ...
         projects={{
           ...
           description: universeCards?.projects.description ?? t('universe.cards.projects.description'),
         ...
         products={{
           ...
           description: universeCards?.products.description ?? t('universe.cards.products.description'),
         ...
         community={{
           ...
           description: universeCards?.community.description ?? t('universe.cards.community.description'),
       ...
       <section className={ctaSectionClassName}>
         <div className="container mx-auto px-4">
           <MarketingCtaBand
             ...
             footer={<ul className="m-0 flex list-none flex-wrap items-center justify-center gap-8 p-0 opacity-40 md:gap-16">
               {ctaFooterItems.map((footerItem) => (
                 <li key={footerItem} className="flex items-center gap-3">
                   <span className="h-1.5 w-1.5 rounded-full bg-primary" aria-hidden="true" />
                   <span className="text-[10px] font-bold uppercase tracking-[0.3em]">{footerItem}</span>
                 </li>
               ))}
             </ul>} />
         </div>
