...
const PUBLIC_PROFILE_DIRECTORY: Record<string, Partial<MockProfileSeed>> = {
  ...
  'citoyen-anonyme': {
    ...
    avatarUrl: null,
    defaultFaction: 'Gardiens des mers',
    impactCreditsBalance: 540,
    city: 'Liege',
    country: 'Belgique',
    tribeIds: ['ocean-guardians'],
  },
  'marie-claire-b': {
...
export const getMockPublicProfile = (
  ...
  return {
    ...
    impactCreditsBalance: directoryEntry.impactCreditsBalance ?? 900,
    totalSeedsContributed: Math.max(180, Math.round((directoryEntry.impactCreditsBalance ?? 900) * 1.15)),
    beesSaved: Math.max(180, Math.round((directoryEntry.impactCreditsBalance ?? 900) * 1.4)),
    honeyGeneratedKg: Number(((directoryEntry.impactCreditsBalance ?? 900) / 3000).toFixed(2)),
    co2CapturedKg: Number(((directoryEntry.impactCreditsBalance ?? 900) / 550).toFixed(2)),
