...
export function HomeFeaturesSection({
  ...
  return (
    <MarketingSection title={title} size="lg" className="relative overflow-hidden py-32">
      ...
      <div className="absolute bottom-0 right-1/4 w-96 h-96 bg-marketing-positive-500/10 rounded-full blur-[128px] -z-10 mix-blend-screen" />
      ...
      <div className="grid gap-8 md:grid-cols-3 relative z-10">
        {featureItems.map((feature) => (
          <Card
            key={feature.title}
            className={cn(
              'group relative border border-marketing-overlay-light/5 bg-marketing-overlay-light/[0.03] backdrop-blur-2xl transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl rounded-[2.5rem] overflow-hidden',
              feature.border,
              feature.glow,
            )}
          >
            <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 bg-gradient-to-b from-marketing-overlay-light/5 to-transparent pointer-events-none" />
