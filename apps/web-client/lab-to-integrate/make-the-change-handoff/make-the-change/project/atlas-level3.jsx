...
const L3_BLOB_VERTICES = [
  [40, 80], [120, 38], [220, 32], [310, 70],
  [350, 160], [358, 270], [340, 380], [280, 460],
  [200, 510], [120, 488], [50, 430], [22, 320], [16, 200],
];

function _hash(s){let h=0;for(let i=0;i<s.length;i++)h=((h<<5)-h+s.charCodeAt(i))|0;return Math.abs(h);}
function _mb(seed){let t=(seed+0x6D2B79F5)>>>0;t=Math.imul(t^(t>>>15),t|1);t^=t+Math.imul(t^(t>>>7),t|61);return((t^(t>>>14))>>>0)/4294967296;}
    ...
    case 'book':
      return (
        <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
          <path d="M4 5 a 2 2 0 0 1 2 -2 h 5 v 17 H 6 a 2 2 0 0 1 -2 -2 Z" fill={color} fillOpacity="0.18"/>
          <path d="M20 5 a 2 2 0 0 0 -2 -2 h -5 v 17 h 5 a 2 2 0 0 0 2 -2 Z" fill={color} fillOpacity="0.18"/>
          <path d="M12 4 v 16"/>
        </svg>
      );
    case 'moon':
    ...
    case 'hex':
      return (
        <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="1.6" strokeLinejoin="round">
          <polygon points="12,3 21,7.5 21,16.5 12,21 3,16.5 3,7.5" fill={color} fillOpacity="0.18"/>
    ...
    case 'apple':
      return (
        <svg width={s} height={s} viewBox="0 0 24 24" fill={color}>
          ...
          <path d="M12 8 C 12 5, 14 4, 16 4" stroke="#220606" strokeWidth="1.4" fill="none" strokeLinecap="round"/>
...
function PollinisationCarte({ onTapItem, onTapCenter }) {
  ...
  return (
    <svg viewBox="0 0 380 540" width="100%" height="100%" preserveAspectRatio="xMidYMid meet"
        style={{ display:'block', isolation:'isolate' }}>
      <defs>
        <clipPath id="l3-blob-clip"><path d={blobPath}/></clipPath>
      ...
      <path d={blobPath} fill="#e6ad44" opacity="0.10" filter="url(#l3-glow)">
        <animate attributeName="opacity" values="0.08;0.13;0.08" dur="6s" repeatCount="indefinite"/>
      </path>

      {/* Blob material */}
      <g clipPath="url(#l3-blob-clip)" style={{ isolation:'isolate' }}>
        <rect x="0" y="0" width="380" height="540" fill="#0a0d10"/>
        <image href="assets/tex_relations.png" x="-10" y="-10" width="400" height="560"
            preserveAspectRatio="xMidYMid slice" opacity="0.78"/>
        <image href="assets/tex_relations.png" x="-10" y="-10" width="400" height="560"
        ...
        <image href="assets/tex_solutions.png" x="-30" y="380" width="440" height="220"
            preserveAspectRatio="xMidYMid slice"
            style={{ mixBlendMode:'overlay' }} opacity="0.32"/>
        {/* Central golden glow halo */}
        <circle cx="190" cy="280" r="110" fill="url(#l3-center-glow)"/>
      </g>

      {/* Glowing amber edge */}
      <path d={blobPath} fill="none" stroke="#f0c460" strokeOpacity="0.85" strokeWidth="1.6" filter="url(#l3-glow)"/>
      ...
      <g stroke="#f0c460" strokeOpacity="0.55" strokeWidth="0.8" strokeDasharray="4 5" fill="none">
        {L3_CONTENT.items.map((it, i) => (
          <path key={i} d={`M 190 280 Q ${(190 + it.iconXY[0]) / 2} ${(280 + it.iconXY[1]) / 2 - 18} ${it.iconXY[0]} ${it.iconXY[1] + 18}`}/>
        ))}
      ...
      {L3_CONTENT.items.map(it => {
        ...
        return (
          <g key={it.id} role="button" aria-label={it.title.join(' ')}
            ...
            {/* icon backplate */}
            <circle cx={ix} cy={iy} r="20" fill="rgba(8,10,14,0.85)" stroke={it.iconColor} strokeOpacity="0.7" strokeWidth="1.1" filter="url(#l3-glow)"/>
            <foreignObject x={ix - 12} y={iy - 12} width="24" height="24">
              <div xmlns="http://www.w3.org/1999/xhtml" style={{ width:'100%', height:'100%', display:'grid', placeItems:'center' }}>
                <L3Icon kind={it.icon} size={20} color={it.iconColor}/>
              </div>
            </foreignObject>
            {/* eyebrow label */}
            <text x={ix} y={iy + 32} textAnchor="middle"
                  fill={it.labelColor} fontFamily="Inter, system-ui, sans-serif"
            ...
            {/* title (1-2 lines) */}
            {it.title.map((line, j) => (
              <text key={j} x={tx} y={ty + j * 16} textAnchor="middle"
                    fill="#f6efdc" fontFamily="Inter, system-ui, sans-serif"
      ...
      {/* ─── BioDex (right side) ─── */}
      <g role="button" aria-label="Espèces BioDex liées"
          style={{ cursor:'pointer' }}
          onClick={() => onTapItem && onTapItem({ id:'biodex', type:'biodex', title:['Espèces BioDex'] })}>
        <text x={L3_CONTENT.biodex.labelXY[0]} y={L3_CONTENT.biodex.labelXY[1]} textAnchor="middle"
            fill="#e6ad44" fontFamily="Inter, system-ui, sans-serif"
            fontSize="9.5" fontWeight="700" letterSpacing="1.6"
            style={{ paintOrder:'stroke', stroke:'rgba(0,0,0,0.85)', strokeWidth:'2.2px' }}>
        ...
        {L3_CONTENT.biodex.species.map((sp, i) => {
          ...
          return (
            <g key={sp.id}>
              <circle cx={cx} cy={cy} r="16" fill={sp.dark} stroke={sp.color} strokeOpacity="0.8" strokeWidth="1.2"/>
              <circle cx={cx} cy={cy} r="14" fill={`url(#l3-tint)`} opacity="0.55"/>
              <foreignObject x={cx - 12} y={cy - 12} width="24" height="24">
                <div xmlns="http://www.w3.org/1999/xhtml" style={{ width:'100%', height:'100%', display:'grid', placeItems:'center' }}>
                  <L3Icon kind={sp.id === 'bee' ? 'bee-mini' : sp.id === 'bumble' ? 'bee-mini' : 'flower-mini'} size={18} color={sp.color}/>
                </div>
              </foreignObject>
              <text x={cx} y={L3_CONTENT.biodex.nameY} textAnchor="middle"
                  fill="#dbd2bb" fontFamily="Inter, system-ui, sans-serif"
        ...
        <text x={L3_CONTENT.biodex.moreXY[0]} y={L3_CONTENT.biodex.moreXY[1]} textAnchor="middle"
            fill="#f0c460" fontFamily="Inter, system-ui, sans-serif"
      ...
      <g>
        <text x={L3_CONTENT.toile.labelXY[0]} y={L3_CONTENT.toile.labelXY[1]} textAnchor="middle"
            fill="#f0c460" fontFamily="Inter, system-ui, sans-serif"
            fontSize="9.5" fontWeight="700" letterSpacing="1.6"
            style={{ paintOrder:'stroke', stroke:'rgba(0,0,0,0.85)', strokeWidth:'2.2px' }}>
        ...
        {L3_CONTENT.toile.nodes.map((node, i) => (
          <g key={node.id}
              role="button" aria-label={`Nœud ${node.name}`}
              style={{ cursor:'pointer' }}
              onClick={() => onTapItem && onTapItem({ id:'toile-' + node.id, type:'toile', title:[`Nœud "${node.name}"`] })}>
            <circle cx={node.x} cy={L3_CONTENT.toile.nodesY} r="13" fill="rgba(8,10,14,0.85)" stroke={node.color} strokeOpacity="0.85" strokeWidth="1.1"/>
            <foreignObject x={node.x - 9} y={L3_CONTENT.toile.nodesY - 9} width="18" height="18">
              <div xmlns="http://www.w3.org/1999/xhtml" style={{ width:'100%', height:'100%', display:'grid', placeItems:'center' }}>
                <L3Icon kind={node.icon} size={14} color={node.color}/>
              </div>
            </foreignObject>
            <text x={node.x} y={L3_CONTENT.toile.nameY} textAnchor="middle"
                fill="#dbd2bb" fontFamily="Inter, system-ui, sans-serif"
            ...
            {i < L3_CONTENT.toile.nodes.length - 1 && (
              <g>
                <path d={`M ${node.x + 14} ${L3_CONTENT.toile.nodesY} L ${L3_CONTENT.toile.nodes[i+1].x - 14} ${L3_CONTENT.toile.nodesY}`}
                    stroke="#dbd2bb" strokeOpacity="0.55" strokeWidth="1" fill="none" markerEnd="url(#arrowtip)"/>
              </g>
            )}
          </g>
        ))}
      ...
      <g role="button" aria-label="Parcours guidé : Le rôle des pollinisateurs"
          style={{ cursor:'pointer' }}
          onClick={() => onTapCenter && onTapCenter()}>
        {/* halo */}
        <circle cx="190" cy="280" r="62" fill="#f0c460" opacity="0.18" filter="url(#l3-strong-glow)">
          <animate attributeName="opacity" values="0.16;0.28;0.16" dur="4s" repeatCount="indefinite"/>
        </circle>
        {/* hex badge */}
        <g transform="translate(190 252)">
          <polygon points="0,-22 19,-11 19,11 0,22 -19,11 -19,-11"
              fill="rgba(10,12,14,0.85)" stroke="#f0c460" strokeOpacity="0.9" strokeWidth="1.2"/>
        ...
        {/* Title */}
        <text x="190" y="312" textAnchor="middle"
            fill="#f6efdc" fontFamily="'Cormorant Garamond', serif"
            fontSize="20" fontWeight="500"
            style={{ paintOrder:'stroke', stroke:'rgba(0,0,0,0.85)', strokeWidth:'3px' }}>
          Le rôle des
        </text>
        <text x="190" y="334" textAnchor="middle"
            fill="#f6efdc" fontFamily="'Cormorant Garamond', serif"
            fontSize="20" fontWeight="500"
            style={{ paintOrder:'stroke', stroke:'rgba(0,0,0,0.85)', strokeWidth:'3px' }}>
          pollinisateurs
...
function PreviewSheet({ item, onClose }) {
  ...
  React.useEffect(() => { if (item) setLast(item); }, [item]);
  const c = item || last;
  ...
  return (
    <>
      <div onClick={onClose} style={{
        ...
        transition:'opacity 260ms ease', zIndex:60,
      }}/>
      <div style={{
        ...
        {c && (
          <div style={{
            ...
            <div style={{ fontSize:10.5, letterSpacing:2, color: meta.color, fontWeight:700, marginBottom:6 }}>
              {meta.label}
            </div>
            <div style={{
              fontFamily:"'Cormorant Garamond', serif", fontSize:24, fontWeight:500,
              lineHeight:1.15, color:'#f6efdc', marginBottom:12,
            }}>
              {Array.isArray(c.title) ? c.title.join(' ') : c.title}
            </div>
            <p style={{ margin:'0 0 18px', fontSize:14, lineHeight:1.5, color:'#cfc8b5' }}>
              Aperçu rapide. Ouvrez pour découvrir le contenu complet, les ressources liées et les espèces concernées.
            </p>
            <button onClick={onClose} style={{
              width:'100%', padding:'14px 18px', borderRadius:9999,
              border:'none', cursor:'pointer',
              background:'linear-gradient(180deg, #f4ecd8, #d9cfb0)',
              color:'#1c1a14', fontWeight:600, fontSize:15,
              display:'flex', alignItems:'center', justifyContent:'center', gap:10,
              boxShadow:`0 8px 24px rgba(0,0,0,0.4), 0 0 0 1px ${meta.color}44`,
            }}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#1c1a14" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <polyline points="9 18 15 12 9 6"/>
              </svg>
              {meta.cta}
            </button>
...
// ── Level 3 screen ──────────────────────────────────────────────
function Level3Screen({ onBack, subdomain = { name:'Pollinisation' } }) {
  const [preview, setPreview] = React.useState(null);

  const startGuide = () => setPreview({
    type:'parcours',
    title:['Le rôle des pollinisateurs'],
  });

  return (
    <div style={{ position:'relative', width:'100%', height:'100%', overflow:'hidden', background:'#04060a' }}>
      ...
      <div style={{
        ...
        paddingTop:46, paddingBottom:28,
        ...
        <div style={{ position:'relative', padding:'2px 16px 0' }}>
          <div style={{ display:'flex', alignItems:'flex-start', gap:10 }}>
            ...
            <div style={{ flex:1, textAlign:'center', minWidth:0, padding:'0 4px' }}>
              <div style={{ fontSize:10.5, letterSpacing:2.6, textTransform:'uppercase', color:'#e6ad44', fontWeight:500 }}>
                Relations du vivant
              </div>
              <h1 style={{
                margin:'1px 0 0',
                fontFamily:"'Cormorant Garamond', serif", fontWeight:500,
                fontSize:30, lineHeight:1.05, color:'#f6efdc',
                letterSpacing:'-0.3px',
              ...
              <div style={{
                ...
                color:'#b9b09a',
              }}>Comprendre comment fleurs, insectes et milieux dépendent les uns des autres.</div>
            </div>
            <button
        ...
        <div style={{ padding:'10px 16px 0' }}>
          <div style={{
            ...
            borderRadius:16, padding:'8px 14px 8px 8px',
            boxShadow:'0 6px 20px rgba(0,0,0,0.4)',
          }}>
            <div style={{ position:'relative', width:42, height:42 }}>
              <svg width="42" height="42" viewBox="0 0 42 42">
                <circle cx="21" cy="21" r="17" fill="none" stroke="rgba(255,255,255,0.12)" strokeWidth="3"/>
                <circle cx="21" cy="21" r="17" fill="none" stroke="#f0c460" strokeWidth="3"
              ...
              <div style={{
                ...
                fontSize:10, fontWeight:600, color:'#f6efdc',
              }}>
                <div style={{ lineHeight:1 }}>6/18</div>
                <div style={{ fontSize:7.5, color:'#9a937f', letterSpacing:0.3, marginTop:1 }}>contenus</div>
              </div>
            </div>
            <div>
              <div style={{ fontSize:10.5, color:'#9a937f', lineHeight:1.2 }}>Progression de</div>
              <div style={{ fontSize:11.5, color:'#eae3d2', lineHeight:1.2 }}>ce thème</div>
              <div style={{ fontSize:13, color:'#f0c460', fontWeight:700, marginTop:1 }}>33 %</div>
            </div>
          </div>
        </div>
        ...
        <div style={{ padding:'18px 16px 4px' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'baseline', marginBottom:10 }}>
            <div style={{ fontFamily:"'Cormorant Garamond', serif", fontSize:20, fontWeight:500, color:'#f6efdc' }}>
              Recommandé pour toi
            </div>
            <button onClick={() => setPreview({ type:'reco', title:['Tous les recommandés'] })} style={{
              background:'none', border:'none', cursor:'pointer',
              color:'#9a937f', fontSize:12, fontWeight:500,
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
            <RecoCard
              ...
              imageTexture="assets/tex_alphabet.png"
            ...
            <RecoCard
              ...
              imageTexture="assets/tex_solutions.png"
  ...
  return (
    <button onClick={onClick} style={{
      ...
      <div style={{ padding:'10px 12px 12px' }}>
        <div style={{ fontSize:9.5, letterSpacing:1.6, fontWeight:700, color:typeColor, marginBottom:4 }}>
          {type}
        </div>
        <div style={{ fontSize:12.5, lineHeight:1.25, color:'#eae3d2', fontWeight:500, minHeight:46 }}>
          {title}
        </div>
        <div style={{ marginTop:8, display:'flex', alignItems:'center', gap:6, fontSize:10.5, color:'#9a937f' }}>
...
Object.assign(window, { Level3Screen });
