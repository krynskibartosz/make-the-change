...
const TERRITORIES = [
  {
    ...
    color: '#7ab84a',
    dark: '#1a2a0e',
    texture: 'assets/tex_solutions.png',
    textureOpacity: 0.80, tintOpacity: 0.28, darken: 0.10,
    iconXY: [100, 110], labelXY: [100, 178],
    labelRX: 78, labelRY: 30,
    icon: 'leaf',
    vertices: ['OL_top', 'O_top_SM', 'J_SMR', 'J_SRM', 'OL_mid'],
    stats: [['Catégories', '24'], ['Initiatives', '186'], ['Près de vous', '7']],
  ...
  },
  {
    ...
    color: '#e25555',
    dark: '#2a0c0c',
    texture: 'assets/tex_menaces.png',
    textureOpacity: 0.72, tintOpacity: 0.22, darken: 0.20,
    iconXY: [282, 110], labelXY: [282, 178],
    labelRX: 78, labelRY: 30,
    icon: 'warn',
    vertices: ['O_top_SM', 'OR_top', 'OR_mid', 'J_MRA', 'J_SMR'],
    stats: [['Pressions', '12'], ['Alertes', '34'], ['Suivies', '3']],
  ...
  },
  {
    ...
    color: '#e6ad44',
    dark: '#2d1f08',
    texture: 'assets/tex_relations.png',
    textureOpacity: 0.78, tintOpacity: 0.20, darken: 0.28,
    iconXY: [191, 298], labelXY: [191, 376],
    labelRX: 95, labelRY: 40, big: true,
    icon: 'net',
    vertices: ['J_SMR', 'J_MRA', 'J_ARI', 'J_MRI', 'J_SRM'],
    stats: [['Liens cartographiés', '4 218'], ['Histoires', '92']],
  ...
  },
  {
    ...
    color: '#4d8be0',
    dark: '#091a32',
    texture: 'assets/tex_milieux.png',
    textureOpacity: 0.84, tintOpacity: 0.28, darken: 0.10,
    iconXY: [68, 358], labelXY: [68, 424],
    labelRX: 56, labelRY: 30,
    icon: 'drop',
    vertices: ['OL_mid', 'J_SRM', 'J_MRI', 'O_bot_IM', 'OL_bot'],
    stats: [['Habitats', '38'], ['Saisons suivies', '4']],
  ...
  },
  {
    ...
    color: '#4ebaa9',
    dark: '#0a2a28',
    texture: 'assets/tex_alphabet.png',
    textureOpacity: 0.72, tintOpacity: 0.32, darken: 0.22,
    iconXY: [312, 358], labelXY: [312, 424],
    labelRX: 64, labelRY: 38,
    icon: 'paw',
    vertices: ['J_MRA', 'OR_mid', 'OR_bot', 'O_bot_AI', 'J_ARI'],
    stats: [['Espèces', '1 240'], ['Familles', '86']],
  ...
  },
  {
    ...
    color: '#a8aeb8',
    dark: '#0e1014',
    texture: 'assets/tex_impact.png',
    textureOpacity: 0.85, tintOpacity: 0.18, darken: 0.14,
    iconXY: [191, 484], labelXY: [191, 540],
    labelRX: 70, labelRY: 24,
    icon: 'gauge',
    vertices: ['J_MRI', 'J_ARI', 'O_bot_AI', 'O_bot_Imid', 'O_bot_IM'],
    stats: [['Indice global', '+12 %'], ['Gestes ce mois', '6']],
    cta: 'Voir mon impact',
...
function buildEdgeMap(segments = 7, amplitude = 7.5) {
  ...
  for (const t of TERRITORIES) {
    ...
    for (let i = 0; i < vs.length; i++) {
      const ka = vs[i], kb = vs[(i + 1) % vs.length];
      ...
      const amp = isOuter ? amplitude * 1.10 : amplitude * 0.92;
...
// never as a gap.
const OUTER_RING = [
  'OL_top', 'O_top_SM', 'OR_top', 'OR_mid', 'OR_bot',
  'O_bot_AI', 'O_bot_Imid', 'O_bot_IM', 'OL_bot', 'OL_mid',
];
...
// ── Icons ──────────────────────────────────────────────────────
function TerritoryIcon({ kind, size = 28, color = '#f4ecd8' }) {
  const s = size;
  switch (kind) {
    case 'leaf':
      return (
    ...
    case 'net':
      return (
        <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="1.6" strokeLinejoin="round" strokeLinecap="round">
    ...
    case 'drop':
      return (
        <svg width={s} height={s} viewBox="0 0 24 24">
          ...
          <path d="M9 14.5 C 9 17, 10.5 18, 12 18.2" stroke="#0e2342" strokeWidth="1.3" fill="none" strokeLinecap="round"/>
        </svg>
      );
    case 'paw':
      return (
...
// ── Main carte ─────────────────────────────────────────────────
function VoronoiAtlas({ onPick, glow = 1, animate = true }) {
  ...
  return (
    <svg viewBox="0 0 380 600" width="100%" height="100%" preserveAspectRatio="xMidYMid meet"
      ...
      {TERRITORIES.map((t, idx) => {
        ...
        const pad = 14;
        const rx = bbox.x - pad, ry = bbox.y - pad;
        const rw = bbox.w + pad * 2, rh = bbox.h + pad * 2;
        ...
        return (
          <g key={t.id}
            ...
            {/* Outer ambient halo */}
            <path d={d} fill={t.color} opacity={0.10 * glow} filter="url(#cellGlow)">
              {animate && <animate attributeName="opacity"
                  values={`${0.08*glow};${0.14*glow};${0.08*glow}`}
                  dur={`${6 + (idx * 0.6)}s`} repeatCount="indefinite"/>}
            </path>
            ...
            <g clipPath={`url(#clip-${t.id})`} style={{ isolation:'isolate' }}>
              ...
              <ellipse cx={t.labelXY[0]} cy={t.labelXY[1] - 2}
                  rx={t.labelRX * 1.15} ry={t.labelRY * 1.05}
      ...
      {TERRITORIES.map(t => (
        <g key={t.id + '-stroke'} style={{ pointerEvents:'none' }}>
          <path d={cellPaths[t.id]} fill="none" stroke={t.color} strokeOpacity="0.85" strokeWidth="1.1" filter="url(#cellGlow)"/>
          <path d={cellPaths[t.id]} fill="none" stroke="#ffffff" strokeOpacity="0.18" strokeWidth="0.4"/>
        </g>
      ))}
      ...
      {JUNCTION_KEYS.map((k, i) => {
        ...
        return (
          <g key={k} style={{ pointerEvents:'none' }}>
            <circle cx={p[0]} cy={p[1]} r={11 * glow} fill="#f4d889" opacity="0.32" filter="url(#nodeGlow)"/>
            <circle cx={p[0]} cy={p[1]} r={6 * glow} fill="#f4d889" opacity="0.5" filter="url(#nodeGlow)"/>
            <circle cx={p[0]} cy={p[1]} r="1.8" fill="#fff5d8" opacity="0.92"/>
            {animate && (
              <circle cx={p[0]} cy={p[1]} r="1.8" fill="#fff5d8">
                <animate attributeName="r" values="1.8;3.6;1.8" dur={`${3 + (i * 0.4)}s`} repeatCount="indefinite"/>
                <animate attributeName="opacity" values="0.85;0.2;0.85" dur={`${3 + (i * 0.4)}s`} repeatCount="indefinite"/>
              </circle>
            )}
          </g>
        );
      ...
      <g style={{ pointerEvents:'none' }}>
        {TERRITORIES.map(t => {
          ...
          return (
            <g key={t.id}>
              <circle cx={ix} cy={iy} r={r + 1.5} fill="none" stroke={t.color} strokeOpacity="0.6" strokeWidth="0.9"/>
              <circle cx={ix} cy={iy} r={r} fill="rgba(8,10,12,0.78)" stroke={t.color} strokeOpacity="0.7" strokeWidth="0.9"/>
...
Object.assign(window, { VoronoiAtlas, TERRITORIES, TerritoryIcon, VERTICES });
