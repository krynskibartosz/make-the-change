import { ChevronRight, DollarSign, FileText, Home, ImageIcon, Info, Package } from 'lucide-react'
 ...
 */
const FieldSkeleton: FC<{ label?: boolean; tall?: boolean }> = ({ label = true, tall = false }) => (
  <div className="space-y-2">
    {label && <div className={`h-3.5 w-28 rounded-md bg-muted/30 ${shimmerClasses}`} />}
    <div className={`${tall ? 'h-24' : 'h-10'} w-full rounded-lg bg-muted/30 ${shimmerClasses}`} />
  </div>
)
    ...
    <div className="space-y-4 px-8 pt-4 pb-8">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
        {Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className={`aspect-square rounded-xl bg-muted/30 ${shimmerClasses}`} />
        ))}
      </div>

      {/* Upload area */}
      <div
        className={`flex h-48 items-center justify-center rounded-xl border-2 border-dashed border-muted/40 bg-muted/10 ${shimmerClasses}`}
      >
