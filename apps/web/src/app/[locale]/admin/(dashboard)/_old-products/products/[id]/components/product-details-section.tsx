...
export const ProductDetailsSection: FC<WithAutoSaveProps> = ({ autoSave }) => {
  ...
  return (
    <DetailView.Section
      icon={FileText}
      title={t('admin.products.edit.sections.details', { defaultValue: 'Détails du produit' })}
    >
      <DetailView.Field
      ...
      {pricePoints > 1000 && descriptionValue.trim().length === 0 && (
        <div className="rounded-md border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">
          ℹ️{' '}
          {t('admin.products.edit.messages.premiumDescriptionRequired', {
            defaultValue:
        ...
        <Controller
          ...
          render={({ field }) => (
            <InputV2
              ...
              error={formState.errors.origin_country?.message}
              value={field.value ?? ''}
              placeholder={t('admin.products.edit.placeholders.originCountry', {
                defaultValue: 'France',
              })}
              onBlur={() => {
                field.onBlur()
