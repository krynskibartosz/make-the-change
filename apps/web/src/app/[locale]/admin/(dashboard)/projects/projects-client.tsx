...
const getLocationLabel = (project: Project): string | undefined => {
	if (project.address.city && project.address.country) {
		return `${project.address.city}, ${project.address.country}`;
	}

	return project.address.city || project.address.country || undefined;
};

const getProjectDescription = (project: Project): string | undefined =>
	project.description ||
	(project.long_description_default as string) ||
	undefined;

const buildAdminProjectBadges = (project: Project): ProjectCardBadge[] => {
...
export const ProjectsClient: FC<ProjectsClientProps> = ({ initialData }) => {
	...
	const getProjectCoordinates = (project: Project): [number, number] => {
		...
		const cityCoords = project.address.city
			? cityCoordinates[project.address.city]
			: undefined;
		if (cityCoords) return cityCoords;

		return (defaultCoordinates[
			project.address.country as keyof typeof defaultCoordinates
	...
	const projectLocations: ProjectLocation[] = projectsState.map((project) => ({
		...
		impactMetrics: (project.impact_metrics ||
			{}) as ProjectLocation["impactMetrics"],
		country: project.address.country || "Non spécifié",
		city: project.address.city || "Non spécifié",
		description:
			project.description || (project.long_description_default as string) || "",
	}));
	...
	const renderProjectActions = (project: Project) => (
		<div className="flex items-center gap-2 flex-wrap" data-card-action>
			<Button
				size="sm"
				variant="outline"
				data-card-action
			...
			{view === "map" ? (
						...
						<CardHeader>
							...
							<div className="bg-warning/10 border border-warning/20 rounded-lg p-3 mt-3">
								<p className="text-sm text-foreground">
									<span className="font-semibold">Note :</span> Les coordonnées
									affichées sont approximatives basées sur les villes. Pour des
									coordonnées GPS précises, une intégration avec une
									bibliothèque PostGIS serait nécessaire.
								</p>
							</div>
						</CardHeader>
						<CardContent>
						...
						<Card>
							<CardHeader>
								<div className="flex items-center justify-between">
									...
									<Badge
										variant={
											["active", "funded", "completed"].includes(
				...
				<DataList
					...
					renderItem={(project: Project) => {
						...
						const heroImage =
							project.hero_image_url || project.avatar_image_url;
							...
							<ProjectCard
								...
								model={{
									...
									href: projectId
										...
										: undefined,
									title: project.name || project.name_default || "Projet",
									subtitle: project.slug || undefined,
									description: getProjectDescription(project),
									image: heroImage
										? {
												src: heroImage,
												alt: project.name || project.name_default || "Projet",
											}
										: undefined,
									status: project.status,
									featured: project.featured,
									projectType: formatProjectType(project.type),
									producerName:
										project.producer?.name_default ||
										project.producer_name ||
										undefined,
									locationLabel: getLocationLabel(project),
									progressPercent: Number(project.funding_progress ?? 0),
									currentFundingEuro: Number(project.current_funding ?? 0),
									targetBudgetEuro: Number(project.target_budget ?? 0),
									launchDateLabel: project.launch_date
										? new Date(project.launch_date).toLocaleDateString(locale)
										: undefined,
									badges: buildAdminProjectBadges(project),
								}}
								labels={{
									viewLabel: "",
								...
								slots={{
									footerActions: renderProjectActions(project),
									metaChips: project.id ? (
										<span className="inline-flex items-center px-2 py-0.5 text-xs text-muted-foreground/70">
					...
					renderSkeleton={() => <ProjectCardSkeleton context="admin" />}
				/>
