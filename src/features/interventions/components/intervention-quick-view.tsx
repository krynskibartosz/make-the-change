...
import { BadgeCheck, Clock, MapPin, PackageCheck, AlertTriangle, Image as ImageIcon } from 'lucide-react'
import Image from 'next/image'
import Link from 'next/link'
import { InterventionDetailTabs, type InterventionDetailTab } from './intervention-detail-tabs'
...
function DataRow({ label, value, isWarning = false }: { label: string; value: React.ReactNode; isWarning?: boolean }) {
  return (
...
export function InterventionQuickView({
  ...
  const isToCheck = intervention.isExtra === 'to_check' || !intervention.zoneId || !intervention.phaseId
        ...
        <div className="pt-6 px-4 space-y-8 pb-12">
          <section>
            ...
            <div className="rounded-2xl border border-border bg-surface p-4 mt-2">
              ...
              <DataRow label="Date" value={new Date(intervention.date || new Date()).toLocaleDateString('fr-FR')} />
            ...
            {workEntries.length > 0 ? (
              <div className="space-y-3">
                {workEntries.map((entry) => (
                  <div key={entry.id} className="rounded-xl border border-border bg-surface p-4 flex justify-between items-center">
                    <div>
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-xs text-muted-foreground">{formatAmountNumber(entry.amount)} €</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="pt-6 px-4 space-y-6 pb-12">
          <section>
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-sm font-bold text-primary">{formatAmountNumber(totalExpenses)} €</span>
            </div>
            
            {expenses.length > 0 ? (
              <div className="space-y-3">
                {expenses.map((expense) => (
                  <div key={expense.id} className="rounded-xl border border-border bg-surface p-4 flex justify-between items-center">
                    <div>
                    ...
                    <div className="text-right">
                      <p className="font-bold text-primary">{formatAmountNumber(expense.amount || 0)} €</p>
        ...
        <div className="pt-6 px-4 space-y-6 pb-12">
          <section>
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-sm font-bold text-muted-foreground">{photos.length} photo(s)</span>
            </div>
            
            {photos.length > 0 ? (
              <div className="grid grid-cols-2 gap-3">
                {photos.map((photo) => (
                  <div key={photo.id} className="relative aspect-square rounded-xl overflow-hidden bg-surfaceElevated">
                    {photo.url ? (
  ...
  return (
    <div className="flex flex-col h-full bg-background">
      ...
      <div className="px-4 pt-4 pb-6 bg-surfaceElevated">
        <p className="text-xs font-bold text-primary mb-1 uppercase tracking-wider">{intervention.id}</p>
