...
import { ChevronLeft, User, Phone, Mail, MapPin, Building2, ChevronRight, Euro, FileText } from 'lucide-react'
...
export default function ClientProfilePage() {
  ...
  if (!client) return <div className="p-8 text-center text-muted-foreground">Client non trouvé.</div>
      ...
      <main className="flex-1 p-5 flex flex-col gap-6">
        ...
        <div className="flex flex-col items-center gap-3 py-4">
          ...
          <div className="flex gap-2 mt-4 w-full">
            <a href={`tel:${client.phone}`} className="flex-1 flex items-center justify-center gap-2 bg-primary text-primary-foreground hover:bg-primary/90 py-3 rounded-xl text-sm font-bold transition-colors shadow-sm">
              <Phone className="size-4" /> Appeler
            </a>
            <a href={`mailto:${client.email}`} className="flex-1 flex items-center justify-center gap-2 bg-surface border border-border hover:bg-surface-elevated py-3 rounded-xl text-sm font-bold transition-colors">
        ...
        <div className="flex flex-col gap-3">
          <h3 className="font-bold text-sm uppercase tracking-wider text-muted-foreground ml-1">Chantiers ({projects.length})</h3>
          {projects.length === 0 ? (
            ...
            projects.map(project => (
              <Link key={project.id} href={`/projet-info`} className="bg-surface border border-border rounded-2xl p-4 flex items-center justify-between hover:border-primary/50 transition-colors">
        ...
        <div className="flex flex-col gap-3">
          <h3 className="font-bold text-sm uppercase tracking-wider text-muted-foreground ml-1">Finances</h3>
          <div className="bg-surface border border-border rounded-2xl overflow-hidden flex flex-col">
            ...
            <div className="p-4 border-b border-border/50 flex justify-between items-center bg-surface-elevated/30">
              <span className="text-sm font-semibold text-blue-500 flex items-center gap-1.5"><Euro className="size-4"/> À facturer</span>
              <span className="font-bold text-base text-blue-500">1 420 €</span>
            </div>
            <Link href="/facturation" className="p-3 flex justify-center items-center gap-2 text-xs font-bold text-muted-foreground hover:text-foreground transition-colors bg-background/50">
